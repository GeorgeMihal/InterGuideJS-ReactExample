...
const card = (wrapper: HTMLElement, control: CardControl) => {
  hydrateRoot(wrapper, <DefaultCard control={control} title={
    <>
      <Text>Всем привет! Сейчас я вам расскажу про библиотеку InterGuideJS и её возможности</Text>
      <Text>Это шаг номер {control.stepNumber}, а всего их {control.stepsCount}</Text>
    </>
  } />)
}
...
const card2 = (wrapper: HTMLElement, control: CardControl) => {
  hydrateRoot(wrapper, <DefaultCard control={control} title={
    <>
      <Text>Информацию для вывода номера шага или их колличества вы можете взять в параметре control</Text>
...
const card3 = (wrapper: HTMLElement, control: CardControl) => {
  hydrateRoot(wrapper, <DefaultCard hasButtons={false} control={{
    ...control,
    next: undefined,
    prev: undefined,
  }} title={
...
const card5 = (wrapper: HTMLElement, control: CardControl) => {
  hydrateRoot(wrapper, <DefaultCard control={control} title={
    <>
      <Button color="purple" type='dashed' onClick={control.deactivate}>Не нажимай на эту кнопку. Придётся начинать всё с начала :/</Button>
    </>
  } />)
}
...
const card6 = (wrapper: HTMLElement, control: CardControl) => {
  hydrateRoot(wrapper, <DefaultCard control={control} title={
    <>
      <Text>Функция control.next доступна только если в действующем шаге активирован флаг nextButton</Text>
      <Text>Функция control.prev доступна только если в предыдущем шаге активирован флаг nextButton</Text>
    </>
  } />)
}
...
const card8 = (wrapper: HTMLElement, control: CardControl) => {
  hydrateRoot(wrapper, <DefaultCard hasButtons={false} control={control} title={
    <>
...
const card11 = (wrapper: HTMLElement, control: CardControl) => {
  hydrateRoot(wrapper, <DefaultCard control={control} title={
    <>
      <Text>У карточек могут быть разные направления расположения, это настраивается в поле direction у поинта</Text>
...
const card19 = (wrapper: HTMLElement, control: CardControl) => {
  hydrateRoot(wrapper, <DefaultCard control={control} title={
    <>
      <Text>Если поле nextButton у шага выставлено в false, то перейти на следующий шаг можно по нажатию на сам элемент</Text>
    </>
  } />)
}
...
const card21 = (wrapper: HTMLElement, control: CardControl) => {
  hydrateRoot(wrapper, <DefaultCard control={control} title={
    <>
      <Text>С помощью поля disable у поинта, можно заблокировать взаимодействие с элементом</Text>
    </>
  } />)
}
...
const card23 = (wrapper: HTMLElement, control: CardControl) => {
  hydrateRoot(wrapper, <DefaultCard control={control} title={
    <>
      <Text>Сценарий может переходить на следующий шаг при появлении всех элементов из списка nextStepElements</Text>
    </>
  } />)
}

const card24 = (wrapper: HTMLElement, control: CardControl) => {
  hydrateRoot(wrapper, <DefaultCard control={control} hasButtons={false} title={
    <>
      <Text>Поинт может становиться активным при появлении всех элементов из списка requiredElements</Text>
    </>
  } />)
}
